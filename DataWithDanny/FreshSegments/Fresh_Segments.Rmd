...
```
0 for interest_id, 7 for map.id
...
To get all columns, filtered for only id 21245, from both tables we use INNER JOIN.
```{sql, connection = con}
...
```
Note we get one record with NULL time stamps values, which wont disrupt our analysis unless we are interested in them. Also, there's no convenient way to exclude one column, like id, from a * query. So I had to list each column name.
...
```
Cumulative pct value passes 90% by 6 months and above. Rest have relatively low clicks and impressions. 
...
The more segments there are, the harder it is to reach these abbritrary thresholds. 
