...
0 for interest\_id, 7 for map.id
...
To get all columns, filtered for only id 21245, from both tables we use
INNER JOIN.
...
Note we get one record with NULL time stamps values, which wont disrupt
our analysis unless we are interested in them. Also, there’s no
convenient way to exclude one column, like id, from a \* query. So I had
to list each column name.
...
We’ll confirm with the \_month column
...
Cumulative pct value passes 90% by 6 months and above. Rest have
relatively low clicks and impressions.
...
110 observations would be removed.
...
these abbritrary thresholds. Removing an interest with 14 months present
...
cte_filter AS ( 
    ...
    GROUP BY total_months, me.month_year
